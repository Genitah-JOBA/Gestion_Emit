...
export function FilieresPage() {
  return <CrudResource
    titre="Filières" sousTitre="Filières de formation de l'EMIT" endpoint="filieres"
...
export function EtudiantsPage() {
  return <CrudResource
    ...
    derive={(form, ref) => {
      ...
      // Matricule auto à la création : 3 chiffres + lettre filière + 2 chiffres année.
      if (!form.id) {
        const fil = (ref.filieres || []).find((f) => String(f.id) === String(form.filiereId));
      ...
      // Genre automatique pour les majeurs (depuis le CIN).
    ...
    columns={[
      { key: 'matricule', label: 'Matricule', render: (r) => <span className="badge">{r.matricule}</span> },
